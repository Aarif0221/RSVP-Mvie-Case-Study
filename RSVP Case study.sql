...
SELECT Sum(CASE
       ...
       Sum(CASE
             ...
             ELSE 0
           END) AS NULL_COUNT_worldwide_gross_income,
       Sum(CASE
...
FROM   movie; 
-- country,worldwide_gross_income,languages and production_company have NULL values.
...
-- Highest number movies released in year 2017.
...
-- 1059 movies were produced in the USA OR INDIA in the year of 2019.
...
+---------------+-------------------+ */
-- Type your code below:
-- Finding the avg duration of movies belongs to genre:
...
-- Action genre has the maximum duration of 112.88 seconds.
...
-- Thriller movies has a movie_count 1484 and Rank=3.
...
-- Top Three movies having avg_rating >= 9.8
...
-- Median rating 7 is the highest number.
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
WITH production_company_hit_movie_summary AS (SELECT production_company, Count(movie_id) AS MOVIE_COUNT,
...
--  Dream Warrior Pictures and National Theatre Live production houses has produced the most number of hit movies.
...
-- Top 3 genres are drama, action and comedy during March 2017 in the USA and had more than 1,000 votes
...
-- 8 movies which begin with the word "The" .
-- Brighton Miracle has average rating > 8 i.e 9.5
...
-- German movies recieves more votes than Italian movies.
...
-- height_nulls, date _of_birth_nulls, known_for_movies_nulls these columns contains NULLS.
...
-- James Mangold , Joe Russo and Anthony Russo are top three directors in the Top 3 genres whose movies have an avg rating > 8 .
...
-- Marvel Studios, Twentieth Century Fox and Warner Bros these are Top 3 production houses based on the number of votes received by their movies.
...
-- Parvathy Thiruvothu, Susan Brown and Amanda Lawrence these are top 3 actresses based on number of Super Hit movies.
